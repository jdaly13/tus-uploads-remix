...
export async function handleTusRequest(request:Request, method: string, id?:string, fileType?: string, nameOfBucket?: string) {
...
async function handleHeadRequest(request:Request) {
  const url = new URL(request.url);
  const fileId = url.pathname.split("/").pop();
  const bucketName = url.searchParams.get("bucket");
